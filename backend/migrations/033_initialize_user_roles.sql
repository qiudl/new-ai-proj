-- 角色权限关联表初始化
-- 文件: 033_initialize_user_roles.sql
...
-- 创建一个管理员用户（如果不存在）
INSERT INTO company_users (name, email, role_id, department, position, status)
SELECT 
    'System Admin',
    'admin@system.com',
    r.id,
...
SELECT 
    ...
    'System initialization - automatic role assignment',
    (SELECT id FROM company_users WHERE email = 'admin@system.com' LIMIT 1),
    CURRENT_TIMESTAMP
FROM company_users cu
JOIN company_roles r ON cu.role_id = r.id
