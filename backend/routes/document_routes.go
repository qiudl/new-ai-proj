...
func registerWorkNotesRoutes(authorized *gin.RouterGroup, app ApplicationInterface) {
	println("[DEBUG] registerWorkNotesRoutes called...")
	...
	// 工作笔记文件夹路由
	println("[DEBUG] About to register work note folder routes...")
	workNoteFolders := authorized.Group("/work-note-folders")
	{
		folderHandler := app.GetWorkNoteFolderHandler()
		if folderHandler == nil {
			println("[ERROR] WorkNoteFolderHandler is nil! Skipping folder routes registration")
			return
		}
		println("[DEBUG] WorkNoteFolderHandler is available, registering routes...")
