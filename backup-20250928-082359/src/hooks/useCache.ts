...
interface CacheOptions {
  ...
  // Enhanced options
  enableDependencyTracking?: boolean; // 启用依赖追踪
  autoInvalidate?: string[]; // 自动失效的相关键
  tags?: string[]; // 缓存标签
  priority?: 'low' | 'normal' | 'high'; // 缓存优先级
  onCacheEvent?: (event: CacheEvent) => void; // 缓存事件回调
  useEnhancedCache?: boolean; // 是否使用增强缓存系统
}
...
export function useCache<T>(
  ...
  const fetchData = useCallback(async (forceRefresh = false): Promise<T | null> => {
    ...
    if (!forceRefresh) {
      ...
      if (useEnhancedCache) {
        cachedData = await enhancedCacheManager.get<T>(key);
        cacheHit = cachedData !== null;
      } else {
        cachedData = globalCache.get<T>(key);
        cacheHit = cachedData !== null;
      }
      
      // Emit cache event
      const duration = Date.now() - startTime;
      cacheEventSystem.emitGet(key, cacheHit, duration, undefined, 'useCache');
      ...
      try {
        if (useEnhancedCache) {
          await enhancedCacheManager.set(key, result, {
            ...
            tags,
            priority,
            registerDependencies: enableDependencyTracking
          });
  ...
  }, [key, ttl, useEnhancedCache, tags, priority, enableDependencyTracking, onCacheEvent]);

  const invalidate = useCallback(async () => {
    if (useEnhancedCache) {
      await enhancedCacheManager.delete(key);
      
      // Handle auto invalidation
      if (autoInvalidate.length > 0) {
        const invalidationPromises = autoInvalidate.map(relatedKey => 
          enhancedCacheManager.delete(relatedKey)
        );
  ...
  }, [key, useEnhancedCache, autoInvalidate]);
  ...
  const mutate = useCallback(async (newData: T | null) => {
    ...
    if (newData !== null) {
      try {
        if (useEnhancedCache) {
          await enhancedCacheManager.set(key, newData, {
            ttl,
            tags,
            priority,
            registerDependencies: enableDependencyTracking
  ...
  }, [key, ttl, useEnhancedCache, tags, priority, enableDependencyTracking]);
  ...
  return {
    ...
    cacheStats: useEnhancedCache ? enhancedCacheManager.getStats() : globalCache.getStats(),
    clearCache: useEnhancedCache ? 
      () => enhancedCacheManager.clear() : 
      () => globalCache.clear(),
    // Enhanced features
    invalidateByTags: useEnhancedCache ? 
      (tags: string[]) => enhancedCacheManager.invalidateByTags(tags) :
      undefined,
    getCacheEvents: () => cacheEventSystem.getEventHistory('get', 10)
  };
}
