...
-- 创建ltree索引以优化层级查询性能
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_path_gist ON tasks USING GIST (path);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_path_btree ON tasks USING BTREE (path);

-- 创建用于子节点查询的索引
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_path_ancestors ON tasks USING GIST (path gist_ltree_ops(siglen=64));

-- 创建复合索引用于常见查询
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_project_path ON tasks (project_id, path) WHERE deleted_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_path_status ON tasks (path, status) WHERE deleted_at IS NULL;
