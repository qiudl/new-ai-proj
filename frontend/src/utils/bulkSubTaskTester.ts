...
export class BulkSubTaskTester {
  ...
  static testTaskNameGeneration(): { passed: boolean; results: any[] } {
    ...
    // 测试1: 正常情况
    try {
      const name = generateSubTaskName(123, 0, '测试任务', 31);
    ...
    try {
      ...
      const name = generateSubTaskName(999, 99, longTitle, 52);
    ...
    try {
      ...
      const name = generateSubTaskName(1, 0, specialTitle, 1);
  ...
  static testEdgeCases(): { passed: boolean; results: any[] } {
    ...
    try {
      ...
      const endName = generateSubTaskName(1, 0, '年末任务', yearEndWeek);
      const startName = generateSubTaskName(1, 0, '年初任务', yearStartWeek);
    ...
    try {
      ...
      const name = generateSubTaskName(1, 0, unicodeTitle, 1);
      results.push({
        test: 'Unicode字符处理',
        input: unicodeTitle,
        output: name,
  ...
  static testPerformance(): { passed: boolean; results: any[] } {
    ...
    try {
      ...
      for (let i = 0; i < taskCount; i++) {
        generateSubTaskName(123, i, `性能测试任务${i}`, 31);
      }
