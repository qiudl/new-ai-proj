...
describe('TokenUsageDisplay', () => {
  ...
  it('应该支持不同的尺寸', () => {
    ...
    // 检查是否应用了小尺寸样式
    expect(screen.getByText('Token使用详情').closest('.ant-card')).toHaveClass('ant-card-small');
  ...
  it('应该在简化模式下显示tooltip', () => {
    ...
    expect(tokenElement.closest('[title]')).toHaveAttribute('title', '输入: 1,000 | 输出: 500');
  });
  ...
  it('应该支持自定义类名', () => {
    ...
    expect(screen.getByText('1,500').closest('.custom-token-display')).toBeInTheDocument();
  });
