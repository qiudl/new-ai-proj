...
import { Card, Select, Button, Space, Tooltip, Switch, Slider, Alert, Spin } from 'antd';
import {
  ZoomInOutlined,
  ZoomOutOutlined,
  ReloadOutlined,
...
interface DependencyGraphVisualizationProps {
  ...
  onCreateDependency?: (sourceId: number, targetId: number) => void;
...
const DependencyGraphVisualization: React.FC<DependencyGraphVisualizationProps> = ({
  ...
  const getNodeStyle = useCallback((node: DependencyGraphNode) => {
    ...
    return baseStyle;
  }, [visualConfig.highlightCriticalTasks, selectedNode]);
...
setSimulation(sim as unknown as d3.Simulation<DependencyGraphNode, undefined>);
  ...
  }, [dependencyGraph, graphConfig, visualConfig, getNodeStyle, getEdgeStyle, onNodeClick, onEdgeClick]);
