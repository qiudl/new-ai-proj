...
export interface WorkNoteFolderStats {
  id: number;
  name: string;
  description?: string;
  color?: string;
...
class WorkNoteFolderService {
  ...
  // 获取文件夹统计信息
  async getFolderStats(id: number): Promise<WorkNoteFolderStats> {
    try {
      const response = await axios.get<APIResponse<WorkNoteFolderStats>>(
...
export const workNoteFolderService = new WorkNoteFolderService();