  ...
  async getFolderTree(visibility?: string): Promise<FolderTreeResponse> {
    try {
      ...
      if (!response || !response.data) {
        console.warn('Empty response from folder tree API');
      ...
      if (!result) {
        console.warn('No data in folder tree response');
        return { tree: [], total_count: 0 };
      }
      
      // 确保树形数据是数组
      if (!Array.isArray(result.tree)) {
        console.warn('Folder tree data is not an array:', result.tree);
        return { tree: [], total_count: result.total_count || 0 };
      }
