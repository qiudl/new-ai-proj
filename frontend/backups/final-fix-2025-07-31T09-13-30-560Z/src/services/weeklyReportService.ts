...
class WeeklyReportService {
  ...
  async getWeeklyReport(startDate?: string, endDate?: string): Promise<WeeklyReportData> {
    try {
      ...
      if ((response as any).success === false || (response as any).error) {
        console.warn('⚠️ 周报API返回错误响应:', response);
        return this.getDefaultWeeklyReportData();
      }
      ...
      // 如果错误对象中包含响应数据，检查是否是API错误格式
      if (error?.response?.data && typeof error.response.data === 'object') {
        const errorData = error.response.data;
        if (errorData.success === false || errorData.error) {
          console.warn('🔧 API返回业务错误，使用默认数据');
          return this.getDefaultWeeklyReportData();
        }
        // 如果响应数据看起来像是正常的周报数据，尝试转换（可能是HTTP状态码误报）
        if (errorData.weekly_stats || errorData.daily_stats) {
          console.warn('🤔 HTTP错误但包含周报数据，尝试处理');
          try {
            return this.transformWeeklyReportData(errorData);
          } catch (transformError) {
            console.error('💥 转换错误响应数据失败:', transformError);
          }
        }
      }
  ...
  private transformWeeklyReportData(backendData: any): WeeklyReportData {
    ...
    if (backendData.success === false || backendData.error) {
      console.warn('⚠️ 后端返回错误响应:', backendData);
      return this.getDefaultWeeklyReportData();
    }
    ...
    if (!weeklyStats) {
      ...
      // 即使没有 weekly_stats，也返回默认数据而不是崩溃
      return this.getDefaultWeeklyReportData();
    }
