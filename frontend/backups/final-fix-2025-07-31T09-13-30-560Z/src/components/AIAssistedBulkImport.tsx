...
const AIAssistedBulkImport: React.FC<AIAssistedBulkImportProps> = ({
  ...
  const handleAIGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        message.success(`AI成功生成${result.tasks.length}个任务 (质量评分: ${result.quality.toFixed(0)}分)`);
      } else {
  ...
  }, [keywords, selectedProvider, maxTasks, projectId, availableProviders, onTasksGenerated, selectedParentTaskId, selectedParentTask]);
  ...
  const handleImportTasks = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        const { total_imported, total_failed, generation_result } = data.data;
        const totalSkipped = generation_result?.metadata?.total_skipped || 0;
        
        if (total_imported > 0) {
          let successMessage = `成功导入 ${total_imported} 个任务`;
          if (totalSkipped > 0) {
            successMessage += `（跳过 ${totalSkipped} 个重复任务）`;
          }
          if (total_failed > 0) {
            successMessage += `，${total_failed} 个任务导入失败`;
          }
          ...
          // 任务已通过AI批量导入API创建，无需再次调用父组件的导入函数
          // 这避免了双重导入问题
        } else if (totalSkipped > 0 && total_imported === 0) {
          message.warning(`所有 ${totalSkipped} 个任务都已存在，没有新任务需要导入`);
        } else if (total_failed > 0 && total_imported === 0) {
          message.error(`导入失败：${total_failed} 个任务无法创建，请检查任务数据或联系管理员`);
        } else {
  ...
  }, [editedTasks, selectedParentTask, selectedProvider, availableProviders, keywords, maxTasks, projectId, selectedParentTaskId]); // onImport is stable from parent
                ...
                <div style={{ marginTop: 8 }}>
                  <TaskSelector
                    ...
                    filterOptions={{
                      ...
                      statusFilter: ['todo', 'in_progress'] // only显示待开始和进行中的任务
                    }}
                    allowClear
                  />
                </div>
