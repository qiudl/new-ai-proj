...
interface ProjectArchivedTasksProps {
  projectId: number;
  projectName: string;
}

const ProjectArchivedTasks: React.FC<ProjectArchivedTasksProps> = ({ projectId, projectName }) => {
  ...
  const fetchArchivedTasks = useCallback(async (page = 1, pageSize = 20) => {
    try {
      setLoading(true);
      const data = await getArchivedTasks(projectId, page, pageSize);
      setTasks(data.tasks);
      setPagination({
        current: data.page,
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      const stats = await getArchiveStatistics(projectId);
      setStatistics(stats);
    } catch (error: any) {
  ...
  const handleUnarchive = async (taskId: number, taskTitle: string) => {
    try {
      ...
      await unarchiveTask(projectId, taskId);
        ...
        locale={{
          emptyText: (
            <Empty
              ...
              description={
                <div>
                  <Text type="secondary">暂无归档任务</Text>
                  <br />
                  <Text type="secondary" style={{ fontSize: '12px' }}>
                    已归档的任务会显示在这里
                  </Text>
...
export default ProjectArchivedTasks;