...
import { useTaskHierarchy, useTaskStatusHierarchy } from '../hooks/useTaskHierarchy';
import TaskHierarchyInfo from './TaskHierarchyInfo';
...
const EnhancedProjectTaskManager: React.FC<EnhancedProjectTaskManagerProps> = ({ 
  ...
  // 层级管理hooks
  const { updateTaskStatus } = useTaskStatusHierarchy(projectId);
  ...
  const [hierarchyDrawerVisible, setHierarchyDrawerVisible] = useState(false);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (hierarchicalTasks) {
        ...
        // 应用排序 - 如果没有手动排序，使用默认智能排序
        let sortedTasks = filteredTasks;
        const finalSortConfig = sortConfig || { field: 'status', order: 'ascend' as const };
        
        if (finalSortConfig) {
          sortedTasks = [...filteredTasks].sort((a, b) => {
            const aValue = getTaskFieldValue(a, finalSortConfig.field);
            const bValue = getTaskFieldValue(b, finalSortConfig.field);
            ...
            if (aValue === null || aValue === undefined) {
              ...
              // 特殊字段排序处理
              switch (finalSortConfig.field) {
                case 'status':
                  // 使用智能排序：逾期 > 进行中 > 待开始 > 已完成 > 已取消
                  const getTaskPriority = (task: any) => {
                    ...
                    if (isOverdue) return 0; // 逾期任务最高优先级
                    ...
                    return statusOrder[task.status as keyof typeof statusOrder] || 5;
                  };
            ...
            return finalSortConfig.order === 'descend' ? -result : result;
          });
        }
  ...
  // 内联编辑相关函数
  const startEditing = useCallback((taskId: number, field: string, currentValue: any) => {
  ...
  const getFieldValue = useCallback((record: Task, field: string) => {
    if (field.startsWith('custom_')) {
      const customKey = field.replace('custom_', '');
      return record.custom_fields?.[customKey];
    }
    return (record as any)[field];
  }, []);

  const renderEditInput = useCallback((field: string, value: any, onChange: (value: any) => void) => {
    const commonProps = {
      size: 'small' as const,
      style: { width: '100%' }
    };
  ...
  const renderDisplayValue = useCallback((field: string, value: any, record: Task) => {
    switch (field) {
      case 'title':
        return <Text style={{ color: value ? undefined : '#999' }}>{value || '点击编辑标题'}</Text>;
      ...
      case 'custom_priority':
        ...
        const pConfig = priorityConfig[value as keyof typeof priorityConfig] || { color: '#d9d9d9', text: '未设置' };
        return <Tag color={pConfig.color}>{pConfig.text}</Tag>;
        
      case 'due_date':
        if (!value) return <Text type="secondary">点击设置</Text>;
        const date = dayjs(value);
        const isOverdue = date.isBefore(dayjs(), 'day');
        const isToday = date.isSame(dayjs(), 'day');
        const isSoon = date.diff(dayjs(), 'day') <= 3 && !isOverdue;
        return (
          <Text style={{ color: isOverdue ? '#ff4d4f' : isToday ? '#fa8c16' : isSoon ? '#fa541c' : undefined }}>
      ...
      default:
        return <Text>{value || '点击编辑'}</Text>;
    }
  }, []);

  const saveEdit = useCallback(async (taskId: number, field: string, value: any) => {
    try {
      setEditLoading(true);
      ...
      if (field === 'due_date' && value) {
        updateData.due_date = dayjs(value).toISOString();
      }
      ...
      const errorMessage = error?.message || error?.error?.message || '更新失败';
      message.error(`更新失败: ${errorMessage}`);
    } finally {
      setEditLoading(false);
    }
  }, [projectId, loadData, cancelEditing]);

  // 快速状态更新（使用层级逻辑）
  const handleQuickStatusUpdate = useCallback(async (taskId: number, newStatus: string) => {
    try {
      await updateTaskStatus(taskId, newStatus);
      await loadData(); // 刷新任务列表
    } catch (error) {
      // 错误已在hook中处理
    }
  }, [updateTaskStatus, loadData]);

  // 显示任务层级信息
  const showTaskHierarchy = useCallback((task: Task) => {
    setSelectedTask(task);
    setHierarchyDrawerVisible(true);
  }, []);

  // 关闭层级信息抽屉
  const closeHierarchyDrawer = useCallback(() => {
    setHierarchyDrawerVisible(false);
    setSelectedTask(null);
  }, []);

  // 计时器启动时的层级处理
  const handleTimerStart = useCallback(async (taskId: number) => {
    try {
      // 从任务列表中找到任务标题
      const task = tasks.find(t => t.id === taskId);
      ...
      // 使用层级服务更新相关任务状态
      await TaskService.updateTasksWithTimer(projectId, taskId, true);
      await loadData(); // 刷新任务列表
    } catch (error: any) {
      message.error(`启动计时器失败: ${error.message}`);
    }
  }, [startTimer, projectId, loadData, tasks]);

  // 渲染内联编辑组件
  const renderInlineEdit = useCallback((taskId: number, field: string, value: any, record: Task) => {
    const isEditing = editingCell?.taskId === taskId && editingCell?.field === field;
    
    if (isEditing) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', width: '100%' }}>
          {editLoading && <Spin size="small" />}
          {renderEditInput(field, editValue, setEditValue)}
          <Button
            type="text"
            size="small"
            icon={<SaveOutlined />}
            onClick={() => saveEdit(taskId, field, editValue)}
            loading={editLoading}
            style={{ color: '#52c41a' }}
          />
          <Button
    ...
    return (
      <div 
        ...
        onClick={() => startEditing(taskId, field, getFieldValue(record, field))}
        title="点击编辑"
      >
        {renderDisplayValue(field, value, record)}
      </div>
    );
  }, [editingCell, editValue, editLoading, startEditing, saveEdit, cancelEditing, renderEditInput, renderDisplayValue, getFieldValue]);
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {renderInlineEdit(record.id, 'title', title, record)}
                  </div>
        ...
        case 'status':
          return {
            ...
            render: (status: string, record: Task) => {
              return renderInlineEdit(record.id, 'status', status, record);
            }};
        ...
        case 'due_date':
          return {
            ...
            render: (dueDate: string, record: Task) => {
              return renderInlineEdit(record.id, 'due_date', dueDate, record);
            }};
              ...
              <Space size="small">
                ...
                <Tooltip title="层级关系">
                  <Button
                    ...
                    onClick={() => showTaskHierarchy(record)}
                  />
                </Tooltip>
                <Tooltip title="删除">
                  <Button
          ...
          if (config.customField) {
            ...
            return {
              ...
              render: (value: any, record: Task) => {
                // 支持内联编辑的自定义字段
                if (['priority', 'progress'].includes(fieldKey)) {
                  return renderInlineEdit(record.id, config.key, value, record);
                }
  ...
  }, [columnConfigs, customFields, navigate, projectId, timerState, timerLoading, handleStartTimer, handlePauseResumeTimer, handleStopTimer, toggleTaskExpansion, renderInlineEdit]);
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          logApiError('PATCH', `/projects/${projectId}/tasks/batch-status`, error);
  ...
  return (
    <div className="enhanced-project-task-manager">
      ...
      {selectedTask && (
        <TaskHierarchyDrawer
          visible={hierarchyDrawerVisible}
          task={selectedTask}
          projectId={projectId}
...
// 任务层级关系抽屉组件
interface TaskHierarchyDrawerProps {
  visible: boolean;
  task: Task;
  projectId: number;
  onClose: () => void;
...
const TaskHierarchyDrawer: React.FC<TaskHierarchyDrawerProps> = ({
  visible,
  task,
  projectId,
  onClose,
  ...
  const { hierarchyInfo, loading } = useTaskHierarchy(projectId, task.id);
    ...
    >
      {loading ? (
        <div style={{ textAlign: 'center', padding: '50px 0' }}>
          <Spin size="large" />
          <div style={{ marginTop: 16 }}>加载层级信息...</div>
        </div>
      ) : hierarchyInfo ? (
        <TaskHierarchyInfo
          ...
          onNavigateToTask={(taskId) => {
            window.open(`/projects/${projectId}/tasks/${taskId}/edit`, '_blank');
          }}
        />
      ) : (
        <div style={{ textAlign: 'center', padding: '50px 0' }}>
