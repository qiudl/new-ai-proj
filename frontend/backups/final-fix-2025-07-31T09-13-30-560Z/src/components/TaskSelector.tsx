...
import { Select, Typography, Tree, Card, Empty } from 'antd';
import {
...
const TaskSelector: React.FC<TaskSelectorProps> = ({
  ...
  const loadTasks = async () => {
    // 防止重复调用
    if (loading) {
      console.log('TaskSelector: 正在加载中，跳过重复请求');
      return;
    }
    ...
    try {
      if (aiMode) {
        ...
        console.log(`TaskSelector: 加载项目 ${projectId} 的任务`);
        ...
        console.log(`TaskSelector: 成功加载 ${availableTasks.length} 个任务`);
      }
    } catch (error: unknown) {
