...
const TimerStatsCard: React.FC<TimerStatsCardProps> = ({ refreshTrigger }) => {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const weeklyReport = await weeklyReportService.getWeeklyReport(
      ...
      // 设置基础统计
      setStats({
        today_total_seconds: (weeklyReport.weeklyStats?.totalHours || 0) * 3600,
        today_formatted_time: TimerService.formatDuration((weeklyReport.weeklyStats?.totalHours || 0) * 3600),
        completed_tasks_today: weeklyReport.weeklyStats?.completedTasks || 0,
        in_progress_tasks: 0,
        recent_tasks: [],
        task_time_breakdown: weeklyReport.taskTimeEntries?.map(entry => ({
          task_id: parseInt(entry.id),
          task_title: entry.taskTitle,
          project_name: entry.projectName,
          total_seconds: entry.duration * 3600,
          formatted_time: TimerService.formatDuration(entry.duration * 3600)
        })) || []
      });
