...
import { EditOutlined, ReloadOutlined, CloudDownloadOutlined } from '@ant-design/icons';
...
interface TaskDocumentEditorProps {
  ...
  initialMode?: 'edit' | 'preview'; // 初始显示模式
}
...
const TaskDocumentEditor: React.FC<TaskDocumentEditorProps> = ({
  ...
  initialMode = 'preview' // 默认以阅读模式显示
}) => {
  const [content, setContent] = useState('');
  const [originalContent, setOriginalContent] = useState('');
  const [preview, setPreview] = useState(initialMode === 'preview');
  const [initialLoad, setInitialLoad] = useState(true); // 标记是否是初始加载
      ...
      try {
        ...
        if (useAdvancedAPI) {
          ...
          if (response && response.data) {
            ...
            // 确保content字段正确提取
            const contentValue = docData.content || '';
            console.log('📄 [增强版API] 文档内容长度:', contentValue.length);
          ...
          if (response && response.data) {
            ...
            // 确保正确提取content字段
            const documentContent = typeof docData === 'object' && docData.content ? docData.content : 
                                   typeof docData === 'string' ? docData : '';
            console.log('📄 加载的文档内容长度:', documentContent.length);
            if (documentContent.length > 0) {
              console.log('📄 文档内容预览:', documentContent.substring(0, 100) + '...');
            } else {
              console.log('⚠️  文档内容为空');
              console.log('🔍 原始响应数据:', docData);
            }
