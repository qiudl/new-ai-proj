...
import { Dropdown, message, Modal, Card, Select, DatePicker} from 'antd';
import { ImportOutlined, MoreOutlined, EditOutlined, AppstoreAddOutlined, CaretRightOutlined, MenuOutlined, InboxOutlined } from '@ant-design/icons';
...
import { TaskCreationNotification, TaskCreationResult } from '../components/TaskCreationNotification';
...
const TasksPage: React.FC = () => {
  ...
  const handleCreateTask = async (taskData: TaskRequest) => {
    ...
    try {
      ...
      const createdTask = await TaskService.createTask(projectId, requestData);
      
      // 检查是否包含文档创建信息 (MVP 2.0)
      let documentInfo: { documentCreated: boolean; documentId?: number; documentPath?: string } = { documentCreated: false };
      
      // 如果返回的响应是新的格式（包含文档信息），解析它
      if (createdTask && typeof createdTask === 'object' && 'document_created' in createdTask) {
        documentInfo = TaskDocumentMVP2Service.extractDocumentInfoFromTaskResponse({ data: createdTask });
        
        // 构建完整的任务创建结果
        const taskCreationResult: TaskCreationResult = {
          task: (createdTask as any).task || createdTask,
          documentCreated: documentInfo.documentCreated,
          documentId: documentInfo.documentId,
          documentPath: documentInfo.documentPath};
        
        // 使用新的通知系统
        TaskCreationNotification.showTaskCreated(taskCreationResult, projectId);
      } else {
        // 旧格式响应，使用传统通知
            ...
            <div>
              ...
              <Table
                ...
                pagination={pagination.total > pagination.pageSize ? {
                  ...
                  showTotal: (total, range) => `第 ${range[0]}-${range[1]} 条/共 ${total} 条`} : false}
