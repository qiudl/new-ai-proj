...
# 完整数据库备份和用户类型迁移脚本
# Full Database Backup and User Type Migration Script
...
backup_critical_tables() {
    ...
    critical_tables=("users" "customers" "company_users" "projects" "tasks")
    
    for table in "${critical_tables[@]}"; do
        table_backup_file="$BACKUP_DIR/table_${table}_${TIMESTAMP}.sql"
        
        if psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "
            \copy (SELECT * FROM $table) TO '$table_backup_file.csv' WITH CSV HEADER;
        " &>/dev/null; then
            log_success "表 $table 备份完成"
        else
            log_warning "表 $table 备份失败或不存在"
        fi
    done
}
...
execute_migration() {
    ...
    if psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f "$migration_file"; then
        ...
        log_error "迁移执行失败！"
        log_error "请检查数据库日志以获取详细错误信息"
        echo
        log_info "如需恢复，可以使用以下命令:"
        echo "psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME < $BACKUP_DIR/full_backup_${DB_NAME}_${TIMESTAMP}.sql"
        exit 1
    fi
}
...
verify_migration_result() {
    ...
    if [[ "$new_columns" == "3" ]]; then
        ...
        log_error "新字段添加失败，期望3个，实际$new_columns个"
    fi
...
1. **使用回滚脚本** (推荐):
   \`\`\`bash
   ./run_user_migration.sh rollback
   \`\`\`

2. **完整数据库恢复** (如果回滚脚本无效):
   \`\`\`bash
   psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME < backups/full_backup_${DB_NAME}_${TIMESTAMP}.sql
   \`\`\`
...
show_completion_summary() {
    ...
    echo
    echo "🔄 如需回滚："
    echo "   ./run_user_migration.sh rollback"
    echo
    echo "📖 详细信息请查看迁移报告文件"
}
