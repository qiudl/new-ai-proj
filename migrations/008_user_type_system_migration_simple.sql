-- Migration: 008 - User Type System Implementation (Simplified)
-- Description: Add user type distinction (system vs company users) and proper permission control
-- Created: 2025-01-22
-- Simplified: Remove problematic sections
...
-- 将现有的 'client' 角色用户标记为企业用户（已弃用：client角色已删除）
-- UPDATE users 
...
-- 将未匹配的企业用户关联到"待分配企业"
UPDATE users 
SET company_id = (SELECT id FROM customers WHERE company_name = '待分配企业' LIMIT 1)
