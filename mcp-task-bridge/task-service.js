...
export class TaskService extends BaseClient {
    ...
    async createTask(title, projectId = 1, options = {}) {
        try {
            ...
            const response = await this.makeRequest('POST', `/projects/${projectId}/tasks`, taskData);
            if (response.success) {
                return {
                    success: true,
                    data: response.data,
    ...
    async listTasks(params) {
        try {
            ...
            const response = await this.makeRequest('GET', url, undefined, queryParams);
            if (response.success && response.data) {
    ...
    async findTaskByName(titlePattern) {
        ...
        try {
            const response = await this.makeRequest('GET', '/tasks', undefined, { search: titlePattern.trim(), page: 1, page_size: 100 });
            if (response.success && response.data) {
                const tasks = response.data.data || [];
                return {
                    success: true,
    ...
    async updateTask(id, updates) {
        try {
            ...
            // 2. 执行更新
            const response = await this.makeRequest('PUT', `/projects/${task.project_id}/tasks/${id}`, updates);
            if (response.success) {
                return {
                    success: true,
                    data: response.data,
    ...
    async getTaskChildren(parentId) {
        try {
            const response = await this.makeRequest('GET', `/tasks/${parentId}/children`);
            if (response.success) {
    ...
    async getTaskTimeline(taskId, projectId = 1, limit = 20, offset = 0) {
        try {
            const response = await this.makeRequest('GET', `/projects/${projectId}/tasks/${taskId}/timeline`, undefined, { limit: limit.toString(), offset: offset.toString() });
            // 处理响应数据
            if (response.success && response.data) {
