...
async function startTask469() {
    ...
    const startTaskCode = `
const { McpTaskService } = require('./task-mcp.ts');
...
startTask();
    ...
    require('fs').writeFileSync('/tmp/start-task-469-exec.cjs', startTaskCode);
    
    const child = spawn('node', ['/tmp/start-task-469-exec.cjs'], {
