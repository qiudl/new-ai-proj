...
export enum TokenRefreshEventType {
  REFRESH_STARTED = 'refresh_started',      // 开始刷新
  REFRESH_SUCCESS = 'refresh_success',      // 刷新成功
  REFRESH_FAILED = 'refresh_failed',        // 刷新失败
  REFRESH_EXPIRED = 'refresh_expired',      // Refresh Token过期
  TOKEN_LOADED = 'token_loaded',            // Token加载成功
  TOKEN_EXPIRED = 'token_expired',          // Token过期
  TOKEN_PERSISTED = 'token_persisted',      // Token持久化保存
  TOKEN_CLEARED = 'token_cleared'           // Token清除
}
...
export interface TokenRefreshEvent {
  ...
  eventType: TokenRefreshEventType;         // 事件类型
  ...
  metadata?: {                              // 元数据
    ...
    timeUntilExpiry?: number;               // 距离过期的时间（秒）
    refreshTokenAge?: number;               // Refresh Token年龄（秒）
...
export interface TokenRefreshStats {
  ...
  successfulRefreshes: number;              // 成功次数
  failedRefreshes: number;                  // 失败次数
  lastRefreshTime?: string;                 // 最后刷新时间
  lastSuccessTime?: string;                 // 最后成功时间
  lastFailureTime?: string;                 // 最后失败时间
  consecutiveFailures: number;              // 连续失败次数
  averageRefreshDuration?: number;          // 平均刷新耗时（毫秒）
...
export interface TokenHealthCheck {
  status: TokenHealthStatus;                // 健康状态
  lastCheck: string;                        // 检查时间
  issues: string[];                         // 问题列表
  recommendations: string[];                // 建议列表
  stats: TokenRefreshStats;                 // 统计信息
}
...
export interface TokenMonitorConfig {
  ...
  enableMetrics?: boolean;                  // 是否启用指标（默认true）
  logFilePath?: string;                     // 日志文件路径
  maxLogSize?: number;                      // 最大日志大小（字节，默认10MB）
  maxEventHistory?: number;                 // 最大事件历史记录数（默认1000）
  alertThreshold?: {                        // 告警阈值
    consecutiveFailures?: number;           // 连续失败次数（默认3）
    failureRate?: number;                   // 失败率（默认0.5，即50%）
  };
}
...
export class TokenRefreshMonitor {
  private config: Required<TokenMonitorConfig>;
  private events: TokenRefreshEvent[] = [];
  ...
  recordEvent(
    ...
    success: boolean,
    metadata?: TokenRefreshEvent['metadata'],
    errorMessage?: string,
    errorCode?: string,
    duration?: number
  ): void {
   ...
   * 更新统计信息
   */
  private updateStats(event: TokenRefreshEvent): void {
    // 更新运行时长
   ...
   * 写入日志文件
   */
  private writeLog(event: TokenRefreshEvent): void {
    try {
      // 检查日志文件大小
   ...
   * 格式化日志行
   */
  private formatLogLine(event: TokenRefreshEvent): string {
    const parts = [
      event.timestamp,
   ...
   * 检查告警条件
   */
  private checkAlerts(event: TokenRefreshEvent): void {
    // 检查连续失败
    if (this.stats.consecutiveFailures >= this.config.alertThreshold.consecutiveFailures) {
      this.triggerAlert(
        'consecutive_failures',
        `Token刷新连续失败 ${this.stats.consecutiveFailures} 次`,
        event
      );
    }
    ...
    if (this.stats.totalRefreshes >= 10) {
      ...
      if (failureRate >= this.config.alertThreshold.failureRate) {
        this.triggerAlert(
          'high_failure_rate',
          `Token刷新失败率过高: ${(failureRate * 100).toFixed(1)}%`,
          event
        );
      }
    }
   ...
   * 触发告警
   */
  private triggerAlert(alertType: string, message: string, event: TokenRefreshEvent): void {
   ...
   * 获取最近的事件
   */
  getRecentEvents(limit: number = 10): TokenRefreshEvent[] {
...
// 全局监控器实例
let globalMonitor: TokenRefreshMonitor | null = null;

/**
 * 获取全局Token刷新监控器
 */
export function getGlobalTokenMonitor(config?: TokenMonitorConfig): TokenRefreshMonitor {
  if (!globalMonitor) {
