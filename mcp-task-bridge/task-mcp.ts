  ...
  async getDetailedTaskInfo(taskId: number): Promise<ApiResponse<any>> {
    try {
      ...
      try {
        ...
        const siblingTasks = allTasks.filter((t: Task) => 
          (t.parent_id === parentId || t.parent_task_id === parentId) && 
          t.id !== taskId
        );
        
        detailedInfo.sibling_tasks = siblingTasks.map((sibling: Task) => ({
          id: sibling.id,
          // 在任务名称前添加ID
      ...
      try {
        ...
        if (childrenResult.success && childrenResult.children) {
          detailedInfo.child_tasks = childrenResult.children.map((child: any) => ({
            id: child.id,
            // 在任务名称前添加ID
