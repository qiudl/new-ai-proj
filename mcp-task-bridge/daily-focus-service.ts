...
export class DailyFocusService extends BaseClient {
     ...
     * 更新今日主要任务信息
     */
    async updateDailyFocusTask(id: number, updates: {
        priority?: 'critical' | 'high' | 'medium' | 'low';
        notes?: string;
        estimated_duration_minutes?: number;
    }) {
    ...
    async clearCompletedTasks(params: {
        ...
        try {
            ...
            const deletePromises = completedTasks.map(task => 
                this.removeDailyFocusTask(task.id)
            );
    ...
    async focusTaskWithTimer(params: {
        ...
        try {
            ...
            const focusTask = focusTasksResult.data.tasks.find(
                task => task.id === params.daily_focus_task_id
            );
     ...
     * 验证今日主要任务请求参数
     */
    private validateDailyFocusTaskRequest(params: any): string[] {
        const errors: string[] = [];
     ...
     * 格式化今日主要任务数据
     */
    private formatDailyFocusTask(task: any) {
        return {
            id: task.id,
