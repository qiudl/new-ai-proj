...
export class ProjectService extends BaseClient {
    ...
    async updateProject(projectId, updates) {
        try {
            ...
            const response = await this.makeRequest('PUT', `/projects/${projectId}`, updates);
            if (response.success) {
                return {
                    success: true,
                    data: response.data,
    ...
    // 复制项目
    // @requiresPermission('create_project')
    async duplicateProject(projectId, newName, includeTasksAndDocs = false) {
        try {
            // 1. 获取原项目信息
    ...
    // 按状态获取项目
    async getProjectsByStatus(status, limit = 10, offset = 0) {
        try {
            const params = {
                status: status,
                limit: limit,
