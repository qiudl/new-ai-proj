...
import { mapMCPStatusToBackend, isValidMCPStatus } from './status-mapping.js';
import fs from 'fs';
import path from 'path';

class EnhancedTaskMCPServer extends TaskMCPServer {
  private documentHandler: EnhancedDocumentHandler;
  ...
  private setupEnhancements() {
    ...
    this.updateTask = async (id: number, updates: any) => {
      // 如果有状态更新，进行映射
      if (updates.status) {
   ...
   * 增强的文档创建方法
   */
  private async enhancedCreateAndAttachDocument(
    taskId: number,
    content: string,
    projectId: number = 1,
    title?: string
   ...
   * 增强的文档获取方法
   */
  private async enhancedGetTaskDocument(
    taskId: number,
    projectId: number = 1
  ) {
