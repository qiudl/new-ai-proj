...
export class WorkNoteService extends BaseClient {
    ...
    // 按标签获取工作笔记
    async getWorkNotesByTags(tags, options = {}) {
        try {
    ...
    // 复制工作笔记
    // @requiresPermission('create_work_note')
    async duplicateWorkNote(id, newTitle) {
        try {
            // 1. 获取原工作笔记
    ...
    // 批量更新工作笔记标签
    // @requiresPermission('update_work_note')
    async batchUpdateTags(workNoteIds, tags) {
        try {
    ...
    // 导出工作笔记
    async exportWorkNote(id, format = 'markdown') {
        try {
