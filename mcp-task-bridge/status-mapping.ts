...
// 状态验证和映射函数
export function mapMCPStatusToBackend(mcpStatus: string): string {
  const normalized = mcpStatus.toLowerCase().trim();
  return TASK_STATUS_MAPPING[normalized] || 'todo';
}

// 检查状态是否有效
export function isValidMCPStatus(status: string): boolean {
  return status.toLowerCase().trim() in TASK_STATUS_MAPPING;
}

// 获取后端状态的所有MCP状态选项
export function getMCPStatusOptions(backendStatus: string): string[] {
...
// 导出类型定义
export type MCPStatus = keyof typeof TASK_STATUS_MAPPING;
export type BackendStatus = 'todo' | 'in_progress' | 'completed' | 'cancelled';
