...
export class TimerService extends BaseClient {
    ...
    async startTimer(taskId, description) {
        try {
            ...
            const response = await this.makeRequest('POST', '/user/timer/start', payload);
            // 兼容后端统一计时器返回格式：顶层即为结果字段，不一定包含 success/data 嵌套
            const timerId = response?.timer_id;
            const startedAt = response?.started_at;
            const ok = response?.success === undefined ? (timerId !== undefined) : !!response?.success;
            if (ok && timerId !== undefined) {
                ...
                return {
                    ...
                    description: description,
                    message: response?.message || `⏱️ 任务 "${taskId}" 开始计时`
                };
            }
            else {
                return {
    ...
    async stopTimer(taskId) {
        try {
            ...
            const response = await this.makeRequest('POST', '/user/timer/stop', {});
    ...
    async getCurrentTimer() {
        try {
            ...
            const resp = await this.makeRequest('GET', '/user/timer/current');
            ...
            if (resp?.success && resp?.data?.active_timers) {
                const activeTimers = resp.data.active_timers.map((timer) => ({
                    id: timer.timer_id,
    ...
    async getActiveTimers() {
        try {
            const resp = await this.makeRequest('GET', '/user/timer/active');
            const timers = resp?.timers || [];
            if (Array.isArray(timers)) {
                const mapped = timers.map((t) => ({
                    id: t.id,
                    task_id: t.target_id || 0,
                    started_at: t.start_time,
                    description: t.description,
            ...
            // 兼容老格式
            if (resp?.success && resp?.data?.active_timers) {
                const activeTimers = resp.data.active_timers;
                return { success: true, data: { active_timers: activeTimers }, active_count: activeTimers.length, timers: activeTimers };
            }
            return { success: true, data: { active_timers: [] }, active_count: 0, timers: [], message: '📝 当前无活跃计时器' };
        }
        catch (error) {
    ...
    // 获取任务计时历史
    async getTaskTimerHistory(taskId, limit = 10, offset = 0) {
        try {
    ...
    // 获取计时器统计信息
    async getTimerStats(taskId, dateFrom, dateTo) {
        try {
            const params = {};
            if (taskId)
    ...
    async updateTimerDescription(timerId, description) {
        try {
    ...
    // 计算工作效率
    async calculateWorkEfficiency(taskId, dateFrom, dateTo) {
        try {
            // 获取任务计时历史
