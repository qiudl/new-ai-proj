 ...
 */
import { TokenRefreshMonitor, TokenRefreshEventType, TokenHealthStatus } from './token-monitor.js';
...
async function testEventRecording() {
    ...
    try {
        ...
        monitor.recordEvent(TokenRefreshEventType.REFRESH_STARTED, true, { expiresAt: new Date().toISOString(), timeUntilExpiry: 60 });
        console.log('2. 记录刷新成功事件...');
        monitor.recordEvent(TokenRefreshEventType.REFRESH_SUCCESS, true, { expiresAt: new Date(Date.now() + 3600000).toISOString(), httpStatus: 200 }, undefined, undefined, 150 // 150ms
        );
        console.log('3. 记录刷新失败事件...');
        monitor.recordEvent(TokenRefreshEventType.REFRESH_FAILED, false, { httpStatus: 500 }, 'Internal server error', '500', 200);
        // 获取最近的事件
...
async function testStatistics() {
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            monitor.recordEvent(TokenRefreshEventType.REFRESH_SUCCESS, true, undefined, undefined, undefined, 100 + i * 10);
        }
        // 记录2次失败刷新
        for (let i = 0; i < 2; i++) {
            monitor.recordEvent(TokenRefreshEventType.REFRESH_STARTED, true);
            monitor.recordEvent(TokenRefreshEventType.REFRESH_FAILED, false, undefined, 'Test error', '500');
        }
