...
interface SeedOptions {
  ...
  timeLogsPerTask?: number;
}

async function createUsers(count: number = 5) {
...
async function createTaskHierarchy(
  ...
  for (let i = 0; i < tasksPerLevel; i++) {
    ...
    // 递归创建子任务
    if (depth > 1) {
      const childTasks = await createTaskHierarchy(
        projectId,
        userIds,
        depth - 1,
        Math.max(1, Math.floor(tasksPerLevel / 2)), // 每层减少任务数
        task.id
      );
...
async function createDocuments(taskIds: number[], projectId: number, count: number = 2) {
  const documents = [];
...
async function createTimeLogs(taskIds: number[], count: number = 15) {
  const timeLogs = [];
...
async function seedDatabase(options: SeedOptions = {}) {
  ...
  try {
    ...
    for (let i = 0; i < projectCount; i++) {
      ...
      const tasks = await createTaskHierarchy(
        project.id,
        userIds,
        maxTaskDepth,
        Math.ceil(tasksPerProject / maxTaskDepth)
      );
      ...
      const documents = await createDocuments(
        taskIds,
        project.id,
        Math.floor(tasks.length * documentsPerTask / 2)
      );
      console.log(`✅ 已创建 ${documents.length} 个文档`);
      
      // 创建时间记录
      console.log(`⏱️  创建时间记录...`);
      const timeLogs = await createTimeLogs(
        taskIds,
        Math.floor(tasks.length * timeLogsPerTask)
      );
...
async function createDemoProject(userIds: number[]) {
  const project = await prisma.project.create({
    data: {
...
function generateTaskCompletionDoc(title: string, assignee: string): string {
  return `# ${title} - 完成报告
