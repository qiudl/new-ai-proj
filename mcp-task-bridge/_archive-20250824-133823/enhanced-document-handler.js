...
export class EnhancedDocumentHandler {
     ...
     * 带重试的请求执行
     */
    async retryableRequest(fn, maxRetries = this.maxRetries) {
        let lastError;
    ...
    async createBatchDocuments(documents) {
        ...
        for (const doc of documents) {
            try {
                const result = await this.createAndAttachDocument(doc.taskId, doc.content, doc.projectId || 1, doc.title);
                results.push(result);
            }
            catch (error) {
                errors.push({
     ...
     * 保存本地备份
     */
    saveLocalBackup(taskId, projectId, content) {
        try {
     ...
     * 获取本地备份
     */
    getLocalBackup(taskId, projectId) {
        try {
            const filename = `task-${taskId}-project-${projectId}.md`;
            const filepath = path.join(this.docsPath, filename);
...
// 导出单例实例
export const documentHandler = new EnhancedDocumentHandler(process.env.API_BASE || 'http://localhost:8080/api/v1');
