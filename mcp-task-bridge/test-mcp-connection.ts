...
async function main() {
  ...
  await runTest('启动任务', async () => {
    ...
    if (!result.success) throw new Error('启动失败');
  });

  await runTest('完成任务', async () => {
    const result = await server.completeTask(taskId);
    if (!result.success) throw new Error('完成失败');
  });
