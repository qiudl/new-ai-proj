...
import { TaskContext, ValidationError, ProcessingError } from '../shared/types';

// 增强版接口定义
export interface EnhancedCreateAndAttachRequest {
  // 原有参数 (保持兼容)
  taskId: number;
  content: string;
  projectId?: number;
...
export type DocumentFormat = 'markdown' | 'txt' | 'html' | 'json';
...
Assignee: {{assignee}}`,
  ...
  async render(templateType: TemplateType, context: any, variables: Record<string, any> = {}): Promise<string> {
  ...
  validateContext(templateType: TemplateType, context: any): ValidationResult {
    const template = this.templates.get(templateType);
    if (!template) {
...
class ContextProcessor {
  private taskContextCache: Map<number, TaskContext> = new Map();
  ...
  private async fetchTaskContextFromService(taskId: number): Promise<TaskContext> {
    // 这里应该调用实际的任务服务API
    // 目前使用模拟数据
    return {
      taskId,
  ...
  private getDefaultContext(taskId: number): TaskContext {
    return {
      taskId,
      title: `Task ${taskId}`,
      description: '',
  ...
  async fillTaskContext(content: string, taskId: number, templateType?: TemplateType): Promise<string> {
    ...
    enrichedContent = enrichedContent.replace(/\{\{task\.(\w+)\}\}/g, (match, field) => {
      return (context as any)[field] || match;
    });
...
class RequestValidator {
  validate(request: EnhancedCreateAndAttachRequest): ValidationResult {
    const errors: string[] = [];
    ...
    // 文档格式验证
    if (request.format) {
      const validFormats: DocumentFormat[] = ['markdown', 'txt', 'html', 'json'];
...
class DocumentProcessor {
  async createDocument(request: EnhancedCreateAndAttachRequest, processedContent: string) {
    // 模拟文档创建
    const document = {
  ...
  async postProcess(document: any, config?: ProcessingConfig) {
    if (config?.auto_save) {
...
export class EnhancedCreateAndAttach {
  ...
  async execute(request: EnhancedCreateAndAttachRequest) {
    try {
      ...
      if (request.autoFillContext) {
        ...
        processedContent = await this.contextProcessor.fillTaskContext(
          processedContent, 
          request.taskId,
          request.templateType
        );
...
// 增强版接口
export async function enhancedCreateAndAttach(request: EnhancedCreateAndAttachRequest) {
  const enhancedService = new EnhancedCreateAndAttach();
